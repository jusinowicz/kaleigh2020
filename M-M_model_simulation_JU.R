#The goal is to have competition emerge by having two consumers of a shared resource
#Model the rate of nitrogen fixation explicitly
#Competition is for phosphorous or light. 
...
#I'm going to take the LV code and try to change it to your MM model: 
## Note:
## Function sigimp passed as an argument (input) to model
## (see also lsoda and rk examples)
MMmod <- function(t, x, parms, input) {
  with(as.list(c(parms, x)), {
    dx <- Vmax*(s/(Km + s)) #Substrate
    ds <- x*C*s #Enzyme
    res <- c(dx, ds)
    list(res)
  })
...
## The parameters
parms <- c(Vmax = 0.1, Km = 100.1, C = 0.1)
## vector of timesteps
times <- seq(0, 200, length = 101)
## Start values for steady state
xstart <- c(s = 500, x = 200)
## Solve model
out <- ode(y = xstart, times = times,
           func = MMmod, parms = parms)
